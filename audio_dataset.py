...
class AudioDataSet:
    ...
    def load(self, file_idxs, sr=None, duration=None):
        if sr is None:
            sr = self.sr
        if duration is None:
    ...
    def load_next(self, batch_num, sr=None, duration=None):
        if self.current_file_idx + batch_num > self.length():
            self.current_file_idx = 0
            
        file_idxs = range(batch_num) + self.current_file_idx
        self.current_file_idx += batch_num
        return load(file_idxs, sr=sr, duration=duration)